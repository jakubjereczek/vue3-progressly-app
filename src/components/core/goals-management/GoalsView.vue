...
const { isFormOpen, editingGoal, draft, goalToDelete, openCreate, openEdit, closeForm, getDraftTargetSeconds } = useGoalsManagement();
...
const goalProgressMap = computed(() => {
  ...
  for (const g of goals.value) {
    ...
    if (target <= 0) { map.set(g.id, 0); continue; }
    const current =
      g.metric === 'count'
...
watch(activeTab, () => { selectedCategoryId.value = null; });
...
<template>
  <Card data-tour="goals" class="p-6 flex flex-col gap-4 rounded-2xl border border-border/40 h-full overflow-hidden">
    ...
    <div class="flex items-center justify-between gap-3 flex-shrink-0">
      <div class="flex items-center gap-0.5 bg-muted/40 rounded-lg p-0.5 border border-border/40">
        <button
          ...
          "
          @click="activeTab = tab.value; viewMode = 'list'"
        >
          {{ t(tab.labelKey) }}
          <span
    ...
    >
      ...
      >{{ t('app.module.goals.filter.all_categories') }}</button>
      <button
    ...
    <GoalsAnalyticsView
      ...
      :filter="activeTab === 'archived' ? 'all' : activeTab"
    />
  ...
  <GoalsDetailSheet
    ...
    :activities="activities"
    :categories="categories"
    @edit="() => { isDetailOpen = false; if (detailGoal) openEdit(detailGoal) }"
    @archive="() => { if (detailGoal) handleArchive(detailGoal) }"
    @unarchive="() => { if (detailGoal) handleUnarchive(detailGoal) }"
    @delete="() => { isDetailOpen = false; if (detailGoal) goalToDelete = detailGoal }"
  />
  ...
  <!-- Delete confirmation -->
  <AlertDialog :open="goalToDelete !== null" @update:open="(v) => { if (!v) nextTick(() => { goalToDelete = null }) }">
    <AlertDialogContent>
      <AlertDialogHeader>
