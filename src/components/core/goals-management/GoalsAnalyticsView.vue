...
function barStatusKey(bar: Bar, item: AnalyticsItem): string {
  ...
  if (isGoalPeriodMet(barValue(bar, item), getTarget(item), isCount(item)))
    return 'app.module.goals.analytics.tooltip.completed';
...
function insightText(insight: Insight): string {
  switch (insight.type) {
    case 'completed':
      return t('app.module.goals.analytics.insight.completed');
    case 'streak':
      return t('app.module.goals.analytics.insight.streak', { n: insight.value });
    case 'on_track':
      return t('app.module.goals.analytics.insight.on_track', { n: insight.value });
    case 'behind':
      return t('app.module.goals.analytics.insight.behind', { n: insight.value });
    case 'great_consistency':
      return t('app.module.goals.analytics.insight.great_consistency', { n: insight.value });
  }
}
</script>
...
<template>
  <TooltipProvider :delay-duration="200">
    <div class="flex flex-col gap-6">
      ...
      <div v-if="allAnalytics.length === 0" class="flex flex-col items-center justify-center py-24 text-center gap-2">
      ...
      <template v-else>
        ...
        <div class="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div v-if="analytics.length >= 2" class="flex items-center gap-4">
            <div class="flex flex-col">
              ...
              <span class="text-2xs text-muted-foreground uppercase tracking-wide">{{
                t('app.module.goals.analytics.summary.goals')
              }}</span>
            </div>
            ...
            <div class="flex flex-col">
              ...
              >
              <span class="text-2xs text-muted-foreground uppercase tracking-wide">{{
                t('app.module.goals.analytics.summary.avg')
              }}</span>
            </div>
            <div class="w-px h-8 bg-border/40" />
            <div class="flex flex-col">
              <span class="text-sm font-semibold tabular-nums">
                <span
                  :style="{
                    color: summary.onTrack === summary.total && summary.total > 0 ? 'var(--color-success)' : undefined,
                  }"
                  >{{ summary.onTrack }}</span
                >
                <span class="text-muted-foreground font-normal text-xs">/{{ summary.total }}</span>
              </span>
              <span class="text-2xs text-muted-foreground uppercase tracking-wide">{{
                t('app.module.goals.analytics.summary.on_track_of')
              }}</span>
            </div>
            <div class="w-px h-8 bg-border/40" />
            <div class="flex flex-col">
              <span
                class="text-sm font-semibold tabular-nums"
                :style="{ color: summary.bestStreak > 0 ? 'var(--color-chart-3)' : undefined }"
                >{{ summary.bestStreak }}</span
              >
              <span class="text-2xs text-muted-foreground uppercase tracking-wide">{{
                t('app.module.goals.analytics.summary.best_streak')
              }}</span>
            </div>
          </div>
        ...
        <!-- Filtered empty -->
        <div v-if="analytics.length === 0" class="flex flex-col items-center justify-center py-16 text-center gap-2">
          ...
          >
            ...
            <div class="px-5 pt-4 pb-0 flex items-start justify-between gap-3">
                  ...
                  <template v-if="getCat(item.goal.category_id)">
                    ...
                    >
                      <span
                        ...
                        :style="{ backgroundColor: getCat(item.goal.category_id)!.color }"
                      />
              ...
              <div class="flex items-center gap-1.5 flex-shrink-0 mt-0.5 flex-wrap justify-end">
                <span
                  class="inline-flex items-center gap-1 text-2xs font-medium px-2 py-1 rounded-md bg-muted/60 border border-border/30"
                >
                  ...
                  >
                    {{
                      item.trend === 'up'
                        ? t('app.module.goals.analytics.trend.improving')
                        : item.trend === 'down'
                          ? t('app.module.goals.analytics.trend.declining')
                          : t('app.module.goals.analytics.trend.stable')
                    }}
                  </span>
                </span>
                <span
                  ...
                  :style="
                    item.isOnTrack
                      ...
                      : {
                          ...
                          borderColor: 'color-mix(in oklab, var(--color-destructive) 25%, transparent)',
            ...
            <div class="px-5 pt-5 pb-4">
              <div class="flex items-start gap-4">
                ...
                <div class="relative flex-shrink-0" style="width: 72px; height: 72px">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center gap-0">
                    ...
                    >
                    <span class="text-2xs text-muted-foreground/50 mt-0.5 uppercase tracking-wide">{{
                      t('app.module.goals.type.per_period')
                    }}</span>
                  </div>
                </div>
                ...
                <div class="flex-1 min-w-0 flex flex-col gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-base font-semibold tabular-nums leading-none" :style="{ color: ringStroke(item) }">
                      ...
                      <span class="text-sm font-normal text-muted-foreground">
                        / {{ fmtTarget(item) }}{{ targetUnit(item) ? ' ' + targetUnit(item) : '' }}</span
                      >
                    </p>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-x-5 gap-y-1">
                    <div>
                      ...
                      <p class="text-sm font-semibold tabular-nums leading-none">
                        ...
                        <span class="text-2xs font-normal text-muted-foreground ml-1"
                          >({{ item.completedPeriods }}/{{ item.totalPeriods }})</span
                        >
                      </p>
                    </div>
                    <div>
                      ...
                      >
                        {{ item.streak }}
                        <span class="text-2xs font-normal text-muted-foreground ml-0.5">{{
                          t('app.module.goals.analytics.periods')
                        }}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p
                      v-if="item.status === 'active' && item.expectedPct > 0"
                      class="text-2xs text-muted-foreground/60 mt-1"
                    >
                      ...
                      <span
                        ...
                        >{{
                          item.isOnTrack
                            ? t('app.module.goals.analytics.pace.on_track')
                            : t('app.module.goals.analytics.pace.behind')
                        }}</span
                      >
                    </p>
                  </div>
              ...
              <div class="mt-4 pt-3.5 border-t border-border/20">
                <p class="text-2xs text-muted-foreground/50 uppercase tracking-widest mb-2">
                  {{ t('app.module.goals.analytics.consistency') }}
                </p>
                  ...
                  <Tooltip v-for="dot in getConsistencyDots(item)" :key="dot.key">
                    ...
                    >
                      ...
                      <p class="text-2xs text-muted-foreground mt-0.5 tabular-nums">
                        {{ barValueFmt(dot, item) }} / {{ fmtTarget(item) }}
                      </p>
                      <p class="text-2xs font-medium mt-0.5" :style="{ color: barStatusColor(dot, item) }">
                        {{ t(barStatusKey(dot, item)) }}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </div>
            ...
            >
              ...
              <TrendingUp
                ...
                style="color: var(--color-chart-3)"
              />
              <Zap v-else class="w-3.5 h-3.5 flex-shrink-0" style="color: var(--color-warning)" />
            ...
            <div class="px-5 pb-4 border-t border-border/20 pt-3.5">
                      ...
                      <div
                        ...
                        :style="{
                          ...
                          height:
                            item.maxBarValue > 0
                              ? Math.max(
                                  (barValue(bar, item) / item.maxBarValue) * 100,
                                  barValue(bar, item) > 0 ? 4 : 0,
                                ) + '%'
                              : '2px',
                    ...
                    >
                      <div class="px-3 py-2 border-b border-border/50 flex items-center justify-between gap-3">
                        ...
                        <span class="text-2xs text-muted-foreground leading-tight">{{
                          barDateRange(bar, item.goal)
                        }}</span>
                      </div>
                      <div class="px-3 py-2 flex flex-col gap-1.5">
                        <div class="flex items-baseline justify-between gap-3">
                          <span class="text-2xs text-muted-foreground">{{
                            t('app.module.goals.analytics.tooltip.value')
                          }}</span>
                        ...
                        <div
                          v-if="!bar.isFuture && getTarget(item) > 0"
                          class="flex items-baseline justify-between gap-3"
                        >
                          <span class="text-2xs text-muted-foreground">{{
                            t('app.module.goals.analytics.tooltip.progress')
                          }}</span>
                        ...
                        <div class="flex items-center justify-between gap-3 pt-0.5 border-t border-border/30">
                          <span class="text-2xs text-muted-foreground">{{
                            t('app.module.goals.analytics.tooltip.status')
                          }}</span>
                          <span class="text-2xs font-semibold" :style="{ color: barStatusColor(bar, item) }">{{
                            t(barStatusKey(bar, item))
                          }}</span>
                        </div>
                      </div>
                    </TooltipContent>
              ...
              <div class="flex items-center justify-between mt-1.5">
                ...
                <span class="text-2xs font-medium tabular-nums" :style="{ color: item.goal.color + 'aa' }"
                  >▸ {{ t('app.module.goals.analytics.now') }}</span
                >
                <span class="text-2xs text-muted-foreground/40 tabular-nums">{{
                  item.bars[item.bars.length - 1]?.label
                }}</span>
              </div>
            </div>
